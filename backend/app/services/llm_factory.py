...
# Fixed import - use relative import for backend structure
from ..core.llm_config import AZURE_CONFIG
...
def _get_azure_client(
    ...
    """Creates or retrieves a cached AsyncAzureOpenAI client."""
...
def get_model(
    ...
    if provider_type == LLMProviderType.AZURE_OPENAI:
        ...
        # Use the mapping from llm_config to get environment variable names
        ...
        if not endpoint or not api_key:
            raise ValueError(f"Endpoint or API key are missing for {model_name}. "
                           f"Expected env vars: {config['endpoint_env']}, {config['key_env']}")
...
def initialize_models(models_to_init: list[Tuple[str, LLMProviderType]]):
    """Optional function to pre-initialize specific models."""
